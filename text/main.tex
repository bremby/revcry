...
same levels of performance \cite{sym-asym}\todo{is StackOoverflow 
citation sufficient?}. For that reason it is not considered 
lightweight. It can, however, be used for so-called hybrid encryption 
schemes, where PKI is used to exchange the secret key for some 
symmetric algorithm.
...
Consider a program $p$ and some values $x$ and $y$. Then $\exe{p}(x) = 
(y)$ represents\footnote{Here we are using the notation from 
\cite{Jones93}.} the execution of program $p$ with $x$ as a given 
parameter and $y$ being the result. So in the case of encryption we have:
...
compute the output, copy the output and then run all its operations 
backwards using the log. The complete proof was also shown by Bennett 
\cite{Bennett73}. Below is a simple example for addition:

\todo{the example is bad. it's just like janus instead of taking pure irreversible language and adding a log}

\begin{lstlisting}[language=c]
  state: x = 3, y = 5, output = 0, output_copy = 0  // initial state
  output += (x + y)                                 // original algorithm
  state: x = 3, y = 5, output = 0, output_copy = 0  // output state
\end{lstlisting}

\begin{lstlisting}[language=c]
  state: x = 3, y = 5, output = 0, output_copy = 0  // initial state
  output += (x + y)                                 // original algorithm
  state: x = 3, y = 5, output = 8, output_copy = 0  // forward run finished
  output_copy += output                             // copy result to a safe location
  state: x = 3, y = 5, output = 8, output_copy = 8  // copy saved
  output -= (x + y)                                 // run original algorithm backwards
...
\todo{citation?}. Janus is a (semi-)high-level C-like language, which 
...
\todo{check that sentence} Furthermore, a new development of this 
language was just under way, so it made sense to stick to a more stable 
and working tool.

\subsubsection{Reversible hardware}

\todo{is this subsubsection worth it? on second thought I doubt it.}
...
way. Reversible code is more predictable in terms of memory management. 
\todo{Ken will complain about that sentence, and I'd agree}. Reversible 
algorithms can follow a much cleaner formal definition that does not 
leak memory contents outside the scope of those functions. \todo{the last sentence gives it away?}

The second goal is to exploit the bijectivity of crypto algorithms. 
Decryption is just an inverse function to encryption, so only of them 
...
Interestingly, both Salsa20 and Chacha20 documents mention 
...
our case. Worthy of notice is also the actual difference in 
implementing the quarterround functions - as we will see later in 
\ref{sec:impl:chacha20}. Even though invertibility is explicitly 
mentioned, neither function is reversible directly, they require some 
manipulation through a temporary local variable.
...
based on user involvement discuss these separately.
...
not be translate that into C without recognizing (or ignoring) it 
...
procedure multiply_three(int a, int b, int c, int result)
  ...
  uncall multiply_proc(a, b, tmp)
  delocal hidden_tmp2 = 0
  delocal hidden_tmp1 = 0
\end{lstlisting}
...
there would be no efficiency decrease. Consider this piece of code, 
...
the 26-line procedure with a 5-line function with same efficiency and 
much better readability.


\subsection{Chacha20}
\label{sec:impl:chacha20}

\todo{explain difference in quarterround functions between Salsa20 and 
Chacha20 here. The difficulty in reversible implementation is 
interesting}
