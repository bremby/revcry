...
\todo{chacha20 quarterround differs!}
...
operation unoperational due to a crash in the LLVM compiler\todo{check 
my zerostack-github issue on this}. The first one, function-based 
...
Starting with a normally compiled binary, we get a normal function 
epilogue (in this case, it is the \code{TEA\_encipher} function):

\begin{verbatim}
  400904:	48 83 c4 20          	add    $0x20,%rsp
  400908:	5d                   	pop    %rbp
  400909:	c3                   	retq   
  40090a:	66 0f 1f 44 00 00    	nopw   0x0(%rax,%rax,1)
\end{verbatim}

The first two instructions revert the stack-related registers 
\code{rsp} and \code{rbp} to their original values and the third one 
performs the return from the function. The last line contains a multi-byte 
\code{nop} instruction and only serves as a padding.


%\begin{verbatim}
  %...
  %400866:       41 81 f8 20 37 ef c6    cmp    $0xc6ef3720,%r8d
  %40086d:       89 57 04                mov    %edx,0x4(%rdi)
  %400870:       75 ae                   jne    400820 <_Z20TEA_encipher_forwardPjS_+0x10>
%### epilogue starts here
  %400872:       f3 c3                   repz retq 
  %400874:       66 66 66 2e 0f 1f 84    data16 data16 nopw %cs:0x0(%rax,%rax,1)
  %40087b:       00 00 00 00 00
%\end{verbatim}

%The \code{retq} instruction performs the return from the function, 
%while the \code{repz} modifier is a bug workaround\footnote{In short, 
%it avoids prediction penalty with some AMD processors. See 
%\cite{repzret} for details.}. The following line is a multi-byte 
%\code{nop} instruction and only serves as a padding.

Now we utilize the function-based cleanup:

\begin{verbatim}
  400934:	48 83 c4 20          	add    $0x20,%rsp
  400938:	5d                   	pop    %rbp
  400939:	48 31 c0             	xor    %rax,%rax
  40093c:	b9 46 00 00 00       	mov    $0x46,%ecx
  400941:	48 8d bc 24 d0 fd ff 	lea    -0x230(%rsp),%rdi
  400948:	ff 
  400949:	f3 48 ab             	rep stos %rax,%es:(%rdi)
  40094c:	48 31 c9             	xor    %rcx,%rcx
  40094f:	31 d2                	xor    %edx,%edx
  400951:	48 31 f6             	xor    %rsi,%rsi
  400954:	c3                   	retq   
  400955:	66 66 2e 0f 1f 84 00 	data16 nopw %cs:0x0(%rax,%rax,1)
  40095c:	00 00 00 00 
\end{verbatim}

The original instructions are still there, but just before the function 
return we see 7 instructions which perform the cleanup. There are 4 
\code{xor} instructions that take a single register twice, thus 
clearing it. The other 3 instructions zero out the stack: the 
\code{mov} instruction sets a counter in the \code{ecx} register, the 
\code{lea} instruction sets the starting address and finally \code{rep 
stos} clears out the stack memory.
...
also be simpler to add to Jana's output; but it doesn't erase registers 
and requires multiple calls to erase stack (one per each stack 
variable)}
